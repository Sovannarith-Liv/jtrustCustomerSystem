...
@app.route('/ITT', methods=['GET', 'POST'])
def ITT():
    itt = (int)(request.args.get('AmountITT'))
    AccountNum = (int)(request.args.get('ITTAccNum'))
    userData = Data.query.filter(Data.accNum == AccountNum).first()
    minItt = userData.minItt
    comPerItt = userData.comPerItt
    maxItt = userData.maxItt

    ittFee = (itt * comPerItt)/100
    print(ittFee)
    if ittFee < minItt: 
        ittFee = minItt
    elif ittFee > maxItt:
    ...
    print(ittFee)
    return render_template("fee.html", ittFee = ittFee, minItt = minItt, maxItt = maxItt)


@app.route('/OTT', methods=['GET', 'POST'])
def OTT():
    ott = (int)(request.args.get('AmountOTT'))
    AccountNum = (int)(request.args.get('OTTAccNum'))
    userData = Data.query.filter(Data.accNum == AccountNum).first()
    minOtt = userData.minOtt
    comPerOtt = userData.comPerOtt
    cabFee = userData.cabFee

    ottFee = (ott * comPerOtt)/100
    print(ottFee)
    if ottFee < minOtt: 
        ottFee = minOtt
    else: 
    ...
    ottFee = ottFee + cabFee
    
    print(ottFee)
    return render_template("fee.html", ottFee = ottFee, minOtt = minOtt, cabFee = cabFee)
